...
func NewRequestResetService(userRepo RequestResetRepo, resetStore redis.PasswordResetStore, notification notification.SendNotification, audit_trail repository.AccountAuditRepository) *RequestResetService {
...
func (r *RequestResetService) RequestReset(ctx context.Context, email string, userAgent string) (string, error) {

	user, err := r.userRepo.GetByEmail(ctx, email)
	if err != nil {
		_, _ = GenerateResetCode()
		return "dummy-code", nil
	}
